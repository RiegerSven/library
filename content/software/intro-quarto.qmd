...
-   A quarto document roughly consists of three parts:
    ...
    -   [Code chunks](#quarto-computations) for programming languages
...
3.  Use the `write_bib()` function from the `knitr` package [@R-knitr].
...
`R` code is included by using curly braces around the **letter r** (i.e.,
```` ```{r} ````)
